...
def load_and_preprocess_stereo_audio(file_path, sample_rate=51200, duration=1.0):
    """
    Load and preprocess a stereo audio file, maintaining both channels separately.
    
    Args:
        file_path: Path to the .wav file
        sample_rate: Target sampling rate (paper uses 51200 Hz)
        duration: Target duration in seconds (paper uses 1.0 second)
    
    Returns:
        Tuple of preprocessed left and right channel audio data
    """
    ...
    # Make sure we actually have stereo audio
    if audio.shape[0] != 2:
    ...
    # Resample both channels if necessary
    ...
    # Ensure exactly 1 second duration for both channels
...
def create_stft_images(left_channel, right_channel, n_fft=2048, hop_length=512):
    """
    Convert both audio channels to STFT grayscale images.
    
    Args:
        left_channel: Left channel audio time series
        right_channel: Right channel audio time series
        n_fft: FFT window size
        hop_length: Number of samples between successive frames
    
    Returns:
        Tuple of STFT magnitude spectrums for left and right channels
    """
...
def visualize_stereo_preprocessing(audio_file_path):
    """
    Visualize the preprocessing steps for a stereo audio file.
    Shows waveforms and STFT images for both channels.
    """
...
def process_stereo_dataset(file_paths):
    """
    Process entire dataset of stereo audio files.
    
    Args:
        file_paths: List of paths to audio files
    
    Returns:
        Processed dataset with left and right channel STFT images
    """
    processed_left = []
    processed_right = []
...
def save_preprocessed_data(file_paths, output_dir='preprocessed_data'):
    """
    Process and save STFT representations for all audio files.
...
# Usage example
audio_files = glob.glob('path/to/your/audio/files/*.wav')
